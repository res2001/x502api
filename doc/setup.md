...
2. Добавить в проект файл линкера нужных библиотек для используемого компилятора:
 ...
 Пример программы для <B> Microsoft Visual С++ </B> находятся  в @lpcie_sdk_dir/examples/msvc, а для <B> Borland C++/Borland C++ Builder </B> --- в @lpcie_sdk_dir/examples/borland_cpp.


Использование библиотеки в проекте на Delphi {#sect_setup_delphi}
=========================================================================

Для написания программ на Delphi с использования API для работы с модулям L502 и E502, необходимо включить в проект программы файл @lpcie_sdk_dir/pas/x502api.pas, а также файл @lpcie_sdk_dir/pas/l502api.pas и/или @lpcie_sdk_dir/pas/e502api.pas, которые представляют собой обертку над библиотеками на *C*. 
...
Файл @lpcie_sdk_dir/pas/lpcieapi.pas предоставляется только для совместимости с проектами для предыдущей версии, реально определения в нем сейчас не испольуются.
...
По сравнению с функциями языка @langname{C} в обертке @langname{C#} сделаны следующие изменения:
 ...
 - Функции `L502.GetSerialList()`, `E502.UsbGetSerialList()`, `L502.GetDevRecordsList()` и `E502.UsbGetDevRecordsList()` возвращают созданный внутри функции динамический массив строк/записей о устройстве (а не заполняет переданный), который уже содержит длину. Поэтому дополнительный параметр размера массива не требуется.
...
Пример программы на C# находится в @filename{@lpcie_sdk_dir/examples/LabView}.
...
Единственным отличием при написании программ на @langname{C/C++} является необходимость подключить lib-файл в соответствии с разрядностью используемого компилятора.
...
Для работы с модулями L502 и E502 используются пакеты:
  ...
  - @filename{lpcie-firm-update} --- Утилита для обновления прошивки ПЛИС модуля L502. Пакет включает в себя последнюю версию прошивки ПЛИС и скрипт lpcie-firm-update-all.sh для обновления прошивок всех найденных устройств L502.
