...
============================================================================
 ...
 При включении автоматического получения адреса модуль выбирает себе локальный (link-local) адрес (и проверяет его уникальность), параллельно выполняя поиск в сети DHCP-сервера. Если DHCP сервер не обнаружен, то используется link-local-адрес, но как только будут обнаружен DHCP-сервер, то предпочтение отдается полученнму адресу от него (в частности при старте в сети с DHCP сервером модуль может некоторое время до получения адреса от него использовать link-local-адрес). Подобный алгоритм используется при  автоматическом полученим адреса в частности в ОС Windows, а также и во многих дистрибутивах Linux (иногда предоставляя возможность отдельного разрешения DHCP и link-local-адреса). Следует также иметь ввиду, что автоматически получаемый адрес модуль проверяет на уникальность в сети, поэтому при существует задержка в несколько секунд от подключения к сети модуля до назначения адреса.
