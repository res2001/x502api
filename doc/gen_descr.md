...
==============================================================
  ...
  5. При циклическом выводе в E502 в отличие от L502 хранение циклического буфера реализовано внутри памяти контроллера ARM Cortex-M4 модуля, что позволяет избежать загрузки интерфейса и ПК во время работы, однако приводит к ограничению размера буфера в 1.5 млн. отсчетов  (суммарно на все разрешенные каналы). Т.е. при работе по сети циклический вывод не влияет на ограничение интерфейса, однако ограничение в суммарную скорость в 5 млн. отсчетов в секунду для содуля сохраняется.
  6. При работе по интерфейсу Ethernet не реализованы функции X502_GetRecvReadyCount() и X502_GetSendReadyCount().
  7. При чтении значений цифровых входов для E502 старшие линии DI14, DI15, DI16 объединены с линиями синхронизации DI_SYN2, CONV_IN и START_IN соответственнно. При этом, так как в обоих модулях линии 17 и 18 объединены с DI_SYN1 и DI_SYN2, то значение 18-ой и 14-ой линии для E502 всегда одинаковы.
...
============================================================================
  Если USB-интерфейс в модуле E502 всегда работает и не требует дополнительной конфигурации, то для работы по интферйсу Ethernet необходимо выполнить настройку параметров интерфейса и включить данный интерфейс. 
  ...
  Основными параметрами для работы по Ethernet являются:
    ...
    - Маска подсети. Определяет какая часть адреса относится к адресу подсети, а какая к адресу устройства. Маска 255.255.255.0 означает, что первые 3 цифры (192.168.0) обозначают адрес сети, последняя цифра (10) - адрес устройства в подсети.
    ...
    - Имя экземляра устройтва. Уникальное имя данного экземпляра в виде строки (до 64 английских символов или 32 русских). Используется в протоколе mDNS-SD для возможности автоматического обнаружения модулей в локальной сети.
  ...
  3. Можно получить нужные параметры с помощью функций E502_EthConfigGetXXX() и установить новые значения с помощью функций E502_EthConfigSetXXX().
  4. После завершения можно записать измененную конфигурацию в модуль с помощью E502_EthConfigWrite(). Модуль сохранит новую конфигурацию в энергонезависимой памяти, отключит Ethernet-интерфейс, после чего снова его переинициализирует уже с новыми параметрами. При этом, если соединение с устройством было выполнено по Ethernet, то для дальнейшей работы нужно разорвать соединение и установить заново (используя новые параметры)
  
  Для избежания изменения конфигурации по сети, конфигурация может быть защищена простым паролем. Пока параоль не установлен в качестве пароля нужно передовать пустую строку. Установка нового пароля выполняется аналогично любым другим изменениям параметров конфигурации (с помощью E502_EthConfigSetNewPassword()). 
