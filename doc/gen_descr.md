...
==============================================================
  ...
  3. Существует ограничение скорости передачи данных для E502. Если L502 позволяет одновременно осуществить ввод и вывод всех данных с аналоговых каналов и цифровых линий на максимальной скорости, то в модуле E502 есть ограничения, которые зависят от используемого интерфейса:
    - при работе по USB суммарная максимальная скорость передачи составляет порядка 4.5 млн. отсчетов в секунду.  Т.е. допустимо использовать например ввод с АЦП и цифровых линий на 2 МГц и при этом вывод только на один канал ЦАП/DOUT на 500 КГц, или на два на 250 КГц. Если идет ввод только с АЦП на 2 МГц, то возможно использовать два канала ЦАП на 1 МГц и т.д.       
    ...
    - при работе по Ethernet (TCP/IP) максимальная скорость уже ограничена скоростью передачи по сети по протоколу TCP. При работе модуля только на ввод скорость ограничена 2.5 млн. отсчетов в секунду. Скорость на вывод и влияние ее на скорость ввода будет уточнена в дальнейшем. Следует также учитывать загрузку самой сети при передаче данных по Ethernet, так как она может сильно влиять на максимальную скорость передачи.
  4. В связи с ограничением скорости передачи в E502 поддерживается возможность задать общий делитель для частоты вывода (тогда как в L502 вывод всегда был на частоте 1 МГц), который можно задать с помощью X502_SetOutFreqDivider() или X502_SetOutFreq().
  5. В настоящее время для E502 не реализован циклический вывод, однако планируется его реализовать для последующих прошивок контроллера. В отличие от L502 хранение циклического буфера планируется реализовать внутри памяти контроллера ARM Cortex-M4 модуля, что позволяет избежать загрузки интерфейса во время работы, однако приводит к ограничению размера буфера в 1.5 млн. отсчетов  (суммарно на все разрешенные каналы).
  6. При работе по интерфейсу Ethernet не реализованы функции X502_GetRecvReadyCount() и X502_GetSendReadyCount().
  7. При чтении значений с цифровых входов для E502 старшие лини несколько иначе объединены с линиями синхронизации по сравнению с L502.
...
Типичная последовательность вызовов имеет следующий вид:
 1. Получение списка серийных номеров при работе с модулями по интерфейсам PCI-Express или USB с использованием функций L502_GetSerialList() и E502_UsbGetSerialList() соответственно. При работе по Ethernet получение списка устройств не реализовано и необходимо перейти сразу к пункту 2.
 2. Если в системе присутствует нужный модуль, создать описатель модуля с помощью X502_Create().
 3. Установить соединение с модулем, передав созданный описатель и серийный номер выбранного устройства в функцию L502_Open() или E502_OpenUsb() для L502 и E502, подключенного по USB, соответственно. Чтобы установить связь с модулем по Ethernet, необходимо использовать функцию E502_OpenByIpAddr(), передав в нее IP-адрес модуля.
 ...
 4. Сделать загруженный сигнал активным с помощью X502_OutCycleSetup().
...
------------------------------------------------------------------------
Хотя самим модулем реализован протокол mDNS-SD для возможности обнаружения устройств в локальной сети, в текущей версии библиотеки нет реализации функций поиска устройств.

Для установки связи с модулем по интерфейсу Ethernet необходимо знать IP-адрес устройства. Именно по IP-адресу устанавливается соединение с устройством, для чего необходимо вызвать функцию E502_OpenByIpAddr().
...
-------------------------------------------------------------
Одним из минусов описанных выше функций является открытия с использованием серийных номеров является то, что необходимо знать, устройству, подключенному по какому интерфейсу, соответстует данный номер, чтобы выбрать соответствующую функцию для открытия устройства. Для избежания этого библиотеки вводят специальный тип #t_x502_devrec, соответтсующий записи об найденном устройстве. Данный тип содержит информацию о найденном устройстве (название, серийный номер, интерфейс, флаги с поддерживающимися возможностями и т.п.), а также содержат всю необходимую информацию о том как установить связь и работать с соответстующим устройством.  Соответственно, от устройства и интерфейса зависет только функции получения записей о устройстве, а установка связи осуществляется общей функцией X502_OpenByDevRecord().

Это позволяет получить все необходимые записи на отдельном этапе и сохранить их в общий массив, а в дальнейшем уже выполнить открытие связи по нужным, не разбирая отдельно, что это за устройство и по какому интерфейсу подключено. 

Особенностью является необходимость пользователю очищать память, выделенную под информацию о том, как установить связь и работать в дальнейшем с модулем, выделенную на этапе инициализации записи об устройстве. Очистка памяти выполняется с помощью X502_FreeDevRecordList(). Запись об устройстве используется исключительно внутри X502_OpenByDevRecord() и при желании может быть освобождена сразу после вызова данной функции, если список записей больше не нужен. Также следует учесть, что перед тем, как проинициализированную ранее запись передавать в функцию для получения или инициализации новой записи, необходимо ее сперва очистить для избежания утечек памяити.

Доступны следующие функции для инициализации записей:
    - L502_GetDevRecordsList() инициализирует записи, соответсвующие подключенным модулям L502 по интерфейсу PCI-Express. По параметрам и использованию аналогична L502_GetSerialList() с учетом необходимости освобождать записи.
    - E502_UsbGetDevRecordsList() инициализирует записи, соответсвующие подключенным модулям E502 по интерфейсу USB. По параметрам и использованию аналогична E502_UsbGetSerialList() с учетом необходимости освобождать записи. 
    - E502_MakeDevRecordByIpAddr() инициализирует запись для установления соединения с модулем E502 по интерфейсу Ethernet. 
    

Пример \filename{examples/msvc/x502_devrec_open} демонстрирует процедуру установки связи с с модулеми через записи о устройствах.
    
Следует отметить, что все функции получения серийных номеров и установления связи с модулеми из предыдущих разделов реально реализованы через описанные в данном разделе функции и являются лишь обертками, созданными для упрощения процедуры поиска и установления связи с устройствами.
...
Плата L502 также позволяет осуществить синхронный вывод на два канала ЦАП и цифровые выводы. При этом для модуля L502 вывод каждого канала осуществляется с частотой в два раза меньшей значения опорной частоты и частоты вывода специально настраивать не нужно. Для модуля E502 можно установить делитель частоты вывода (относительно максимальной равной половине опорной) либо явно с помощью функции X502_SetOutFreqDivider(), либо вызвать функцию X502_SetOutFreq(), чтобы она подобрала такой делитель, чтобы частота была наиболее близка к заданной. При этом частота задается общая для всех потоков вывода.
...
Значение navg задается последним параметром функции X502_SetLChannel(). Значение равное 1 означает отсутствие усреднения. Значения равное 0 означает, что коэффициент усреднения может быть выбран по усмотрению библиотеки. В текущей реализации значение 0 аналогично значению 1, но это может быть изменено в последующих версиях.
  ...
  -------------------------------------------------------------------------
  Для модуля L502, начиная с версии 1.0.4 драйвера и библиотеки введена поддержка циклического вывода на ЦАП и цифровые выводы. Этот режим позволяет загрузить сигнал полностью в буфер внутри драйвера, содержимое которого будет циклически передаваться в модуль L502 без необходимости дальнейшей подкачки.
