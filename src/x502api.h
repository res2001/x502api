...
typedef enum {
    ...
    /** Флаг указывает, что функция должна дождаться полной загрузки сигнала
        и установки сигнала на вывод. Без данного флага функции X502_OutCycleSetup()
        и X502_OutCycleStop() только посылают команду модулю, не дожидаясь реального
        выполнения действий. Флаг необходим при  вызове X502_OutCycleSetup()
        при загрузке первого сигнала до вызова X502_StreamsStart(), если
        необходимо, чтобы выдача первых отсчетов на ЦАП совпала по времени с
        моментом запуска ввода, т.к. иначе передача сигнала в модуль может
        не завершиться к моменту запуска и выдача сигнала начнется с задержкой. 
        При последующих вызовах X502_OutCycleSetup() для смены уже установленного 
        сигнала эта функция ожидает также момента непосредственного начала
        выдачи нового сигнала. При вызове X502_OutCycleStop() с данным флагом
        ожидается момент останова выдачи, что может быть использовано для уверенности,
        что остановка произошла на последней точке и можно вызвать X502_StreamsStop(),
        если необходимо чтобы на выходах осталось именно последнее значение записанных
        сигналов.
        ...
        #X502_OUT_CYCLE_FLAGS_FORCE), но при желании проверку завершения
        можно сделать с помощью отдельной функции X502_OutCycleCheckSetupDone().
...
/***************************************************************************//**
    ...
    @note Данная поддерживается только для E502. Для L502 в текущей прошивке
    ПЛИС она не поддерживается.
...
/***************************************************************************//**
    ...
    быть вызвана после X502_SetRefFreq(), в противном случае полученные делители
...
/***************************************************************************//**
    ...
    функция должна быть вызвана после X502_SetRefFreq(), в противном случае
...
/***************************************************************************//**
    ...
    @param[out] din         При успешном выполнении в этой переменной
                            ...
                            Для E502 биты 15-16 дублируют значения битов 18-17.
...
/***************************************************************************//**
    ...
    Для E502 размер ограничен памятью встроенного контроллера. В текущей прошивке
    циклический режим вывода не реализован. Точное значение будет определено позже.
...
/***************************************************************************//**
    ...
    X502_OutCycleSetup() или завершена ли остановка циклического сигнала после
    вызова X502_OutCycleStop(). По своему назначению аналогична флагу 
    ...
    не выполняются, то функция явно вернет ошибку.
