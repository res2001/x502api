...
/** @brief События поиска сетевых сервисов
    ...
    функцией E502_EthSvcBrowseGetRecord() */
typedef enum {
...
/**************************************************************************//**
    ...
    устройств в локальной сети. Создается при вызове E502_EthSvcBrowseGetRecord()
...
/***************************************************************************//**
    ...
    Данная функция инициализирует запись о устройстве, подключенном по интерфейсу
    Ethernet, соответствющему сетевому сервису, на который указывает переданный
    ...
    о устройтсве, т.е. после вызова данной фунции при желании описатель
...
/***************************************************************************//**
   ...
   При вызове данной функции запускается процесс поиска сервисов в сети, соответствующих
   модулям E502, в локальной сети и создается контекст текущего сеанса поиска.
   Этот контекст используется для дальнейших вызовов E502_EthSvcBrowseGetRecord().
   После завершения поиска должна быть вызвана функция E502_EthSvcBrowseStop().
   Для запуска сеанса необхдим запущенный сервис (демон) обнаружения -
...
/***************************************************************************//**
   ...
   E502_EthSvcBrowseGetRecord() с событием #E502_ETH_SVC_EVENT_ADD, за
   ...
   При желании продолжать вызвать данную функцию для непрерывного отслеживания
   ...
   @param[out]  svc         Если возвращенное событие не равно #E502_ETH_SVC_EVENT_NONE,
                            ...
                            сетевого сервиса, соответствующего указанному события. Этот
   ...
   @param[out]  flags       В данной переменной сохраняются дополнительные коды
                            ...
                            если значение флагов не требуется.
...
/***************************************************************************//**
   ...
   При вызове данной функции процесс поиска сетевых сервисов, соответствющий
   ...
   недействительным.
   Вызовыу E502_EthSvcBrowseStart() всегда должен соответствовать последующий
   вызов E502_EthSvcBrowseStop() для корректного особождения ресурсов.
...
/***************************************************************************//**
    ...
    @param[in] tout    Время ожидания ответа от модуля в случае необходимости
                       выполить запрос для установления адреса.
    @return            Код ошибки
*******************************************************************************/
